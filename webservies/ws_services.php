...
if ($actions == 'addlmslead') {

    $request_body = @file_get_contents('php://input');
    $obj = json_decode($request_body);
    
    // if($obj->Mobile != "" && $obj->Email){
    if (!empty($obj->Mobile) && !empty($obj->Email)) {
        
        $dataCust = array(
            "title" => '',
            "firstName" => ucwords($obj->FirstName ?? ''),
            "MiddleName" => '',
            "lastName" => '',
            "mobileNo" => $obj->Mobile ?? '',
            "email" => $obj->Email ?? '',
        ...
        $InquiryFor = "";
        if($obj->InquiryFor == "Graphic Web UI UX & Multimedia Design"){
            $InquiryFor = "Graphics";    
        } else if($obj->InquiryFor == "3D Animation"){
            $InquiryFor = "Animation";  
        } else if($obj->InquiryFor == "Game Designing"){
            $InquiryFor = "Gaming";  
        }  else if($obj->InquiryFor == "Visual Effects VFX"){
            $InquiryFor = "VFX";  
        } else {
            $InquiryFor = "Graphics";  
        }
        ...
        $rowfilterInq = mysqli_fetch_array(mysqli_query($dbconn, "SELECT Count(*) as count  FROM `lead`  where  customerEntryId='" . $customerEntryId . "'"));
        $noOfInquiry = $rowfilterInq['count'];
        $rowfilterBooked = mysqli_fetch_array(mysqli_query($dbconn, "SELECT Count(*) as count  FROM `lead`  where  customerEntryId='" . $customerEntryId . "' and statusId='3'"));
...
// else if ($actions == 'fb_verify') {
//     $verify_token = 'mycustom80'; // default
    
//     // Accept either hub.mode or hub.mode param naming variations
...
//     }
// }
// else if ($actions == 'fb_receive') {
//     $raw = @file_get_contents('php://input');
//     $payload = json_decode($raw, true);

//     // Try to get lead id from multiple spots (same logic as Laravel)
//     $leadgenId = null;
...
//     if ($leadgenId) {
//         //fb_log("Found raw lead id: {$leadgenId}");
//         // normalize digits
//         $normalized = preg_replace('/\D/', '', $leadgenId);
//         $toFetch = !empty($normalized) ? $normalized : $leadgenId;

//         // call internal fetch (via same file) to keep separation — we'll call internal function below
//         // best approach: call action=fb_fetch internally
//         $_GET['lead_id'] = $toFetch; // ensure available
//         // call fetch function directly:
//         // we'll implement fb_fetch below as a callable
//         // store guid and lead id for use
//         $internal_lead = $toFetch;

//         // call fb_fetch function (defined later)
//         // to ensure function exists, we will call a helper below
//         //fb_log("Calling fb_fetch for id={$internal_lead}");
//         // We implement fetch inline by including the fetch code as a function below and calling it
//         // (see function fb_fetch_lead_details)
//         fb_fetch_lead_details($internal_lead);
//     } else {
//         $output['message'] = 'No lead id found in payload';
//         $output['success'] = '0';
//     }
...
//     $output['success'] = '1';
// }
else if ($actions == 'fb_fetch') {
    $lead_id = $_REQUEST['lead_id'] ?? '';
    if (empty($lead_id)) {
        $output['message'] = 'Missing guid or lead_id';
        $output['success'] = '0';
    }

    fb_fetch_lead_details($lead_id,$dbconn);
    $output['message'] = 'Fetch attempted (check logs)';
    $output['success'] = '1';
}
...
function fb_fetch_lead_details($leadgenId,$dbconn) {
    ...
    // 4) Extract fields to assoc (name => value)
    print_r($leadData['field_data']);
    exit;
    $leadInfo = [];
    ...
    $course = $leadInfo['course'] ?? $leadInfo['inquiry_for'] ?? $leadInfo['interest'] ?? '';
    $remarks = ''; // we will build remarks from other keys if present
