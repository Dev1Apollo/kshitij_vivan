...
if (mysqli_num_rows($datafilter) > 0) {
    ...
    while ($dataresult = mysqli_fetch_array($filterstudent)) {
        $filterEmi = mysqli_query($dbconn, "select * from studentemidetail where studentcourseId='" . $dataresult['studentcourseId'] . "' and isDelete=0 and  stud_id='" . $dataresult['stud_id'] . "' ORDER by STR_TO_DATE(emiDate,'%d-%m-%Y') ASC limit 1");
        ...
        $filterTotalReceiveAmount = mysqli_fetch_array(mysqli_query($dbconn, "select sum(amount) as ReceivedAmount from studentfee where studentcourseId='" . $dataresult['studentcourseId'] . "' and stud_id='" . $dataresult['stud_id'] . "' " . $whereFee . " and feetype = '2' group by stud_id,studentcourseId ORDER by STR_TO_DATE(payDate,'%d-%m-%Y') asc"));
