...
if ($_POST['action'] == 'ListUser') {
    ...
    if ($_REQUEST['branch'] != NULL && isset($_REQUEST['branch'])) {
        $whereEmpId = " and studentadmission.branchId = '" . $_REQUEST['branch'] . "'";
    }
    $where = "";
    $whereEmi = "";
...
//    if (mysqli_num_rows($resultfilter) > 0) {
    ...
    if (mysqli_num_rows($filterstudent) > 0) {
        while ($dataresult = mysqli_fetch_array($filterstudent)) {
            ...
            $filterTotalReceiveAmount = mysqli_fetch_array(mysqli_query($dbconn, "select sum(amount) as ReceivedAmount from studentfee where studentcourseId='" . $dataresult['studentcourseId'] . "' and stud_id='" . $dataresult['stud_id'] . "' " . $whereFee . " and feetype in (2,5) and studentfee.isCancel=0  group by stud_id,studentcourseId ORDER by STR_TO_DATE(payDate,'%d-%m-%Y') asc"));
                ...
                for ($jCounter = 0; $jCounter < sizeof($array[0]); $jCounter++) {
                    if ($jCounter <= 5) {
                        ...
                        $filterstr = mysqli_fetch_array(mysqli_query($dbconn, "SELECT  sum(studentfee.amount) as ReceivedFeeAmount from studentfee where month(STR_TO_DATE(studentfee.payDate,'%d-%m-%Y')) = month(STR_TO_DATE('" . $date . "','%d-%m-%Y')) "
                                        ...
                                        . " and stud_id='" . $getemi['stud_id'] . "' and studentcourseId='" . $getemi['studentcourseId'] . "' and studentfee.feetype = '2' and studentfee.isCancel=0 "
                        ...
                        $filterCurrentMOunthEmi = mysqli_fetch_array(mysqli_query($dbconn, "SELECT  sum(studentemidetail.emiAmount) as emiAmount from studentemidetail where month(STR_TO_DATE(studentemidetail.emiDate,'%d-%m-%Y')) = month(STR_TO_DATE('" . $date . "','%d-%m-%Y')) "
                                        ...
                                        . " and stud_id='" . $getemi['stud_id'] . "'  and studentcourseId='" . $getemi['studentcourseId'] . "'  and studentemidetail.isDelete=0"
