...
if ($_POST['action'] == 'ListUser') {
    ...
    if (isset($_REQUEST['Search_Txt'])) {
        if ($_POST['Search_Txt'] != '') {
            ...
            $where.=" and  inquirySourceName like '%$_POST[Search_Txt]%'";
        }
    }

    $filterstr = "SELECT * FROM `inquirysource`  " . $where . " and isDelete='0' order by  inquirySourceId desc";
    $countstr = "SELECT count(*) as TotalRow FROM `inquirysource` " . $where . " and isDelete='0' ";
...
// echo $filterstr;
    ...
    if (mysqli_num_rows($resultfilter) > 0) {
        ...
        <form role="form" method="POST" action="" name="frmCourseStatus" id="frmCourseStatus" enctype="multipart/form-data">
            <input type="hidden" value="InquirySourceStatus" name="action" id="action">
                ...
                <tbody>
                    ...
                    while ($rowfilter = mysqli_fetch_array($resultfilter)) {
                        ...
                        <tr>
                            <td>
                        <div class="md-checkbox">
                                    <input type="hidden" name="inquirySourceId[]" id="smsid<?php echo $i; ?>" class="md-check"
                                        value="<?php echo $rowfilter['inquirySourceId']; ?> ">
                            ...
                            <td>
                                <div class="form-group form-md-line-input "><?php echo $rowfilter['inquirySourceName']; ?> 
                                </div>
                            </td>
                            ...
                            <td>
                                ...
                                <?php
									if ($rowfilter['istatus'] == 0) { ?>
                                        <a class="btn red"
                                            onClick="javascript: return deletedata('Active', '<?php echo $rowfilter['inquirySourceId']; ?>');"
                                            title="Inactive"><i class="fa fa-times iconshowFirst"></i></a>
                                        <?php } else { ?>
                                        <a class="btn green"
                                            onClick="javascript: return deletedata('Inactive', '<?php echo $rowfilter['inquirySourceId']; ?>');"
                                            title="Active"><i class="fa fa-check iconshowFirst"></i></a>
                                        <?php } ?>
                                    <a class="btn blue" onClick="javascript: return setEditdata('<?php echo $rowfilter['inquirySourceId']; ?>');"  title="Edit"><i class="fa fa-edit iconshowFirst"></i></a> 
    
                                    <a  class="btn blue" onClick="javascript: return deletedata('Delete', '<?php echo $rowfilter['inquirySourceId']; ?>');"   title="Delete"><i class="fa fa-trash-o iconshowFirst"></i></a>
                                </div>
                            </td>
...
if ($_REQUEST['action'] == 'Delete') {
    ...
    $where = ' where inquirySourceId=' . $_REQUEST['ID'];
