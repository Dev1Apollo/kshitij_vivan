...
if ($_POST['action'] == 'ListUser') {
    ...
    if ($_REQUEST['branch'] != NULL && isset($_REQUEST['branch'])) {
        $where.= " and customerentry.employeeMasterId in (select employeeMasterId from employeemaster where employeemaster.branchid=" . $_REQUEST['branch'] . " and istatus=1 and isDelete=0 )";
    }

    $filterstr = "select * from lead INNER JOIN customerentry on lead.customerEntryId = customerentry.customerEntryId " . $where . " and lead.isNewInquiry='0'  order by STR_TO_DATE(lead.inquiryEnterDate,'%d-%m-%Y') DESC";

    $countstr = "SELECT count(*) as TotalRow from lead INNER JOIN customerentry on lead.customerEntryId = customerentry.customerEntryId " . $where . " and lead.isNewInquiry='0'  ";
