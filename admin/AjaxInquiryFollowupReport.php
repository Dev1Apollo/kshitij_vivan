...
if ($_POST['action'] == 'ListUser') {
    ...
    if ($_REQUEST['ToDate'] != NULL && isset($_REQUEST['ToDate']))
        $where.=" and STR_TO_DATE(nextFollowupDate,'%d-%m-%Y')<=STR_TO_DATE('$_REQUEST[ToDate]','%d-%m-%Y')";
    if ($_REQUEST['Employee'] != NULL && isset($_REQUEST['Employee']))
        $where.=" and employeeMasterId='" . $_POST['Employee'] . "'";

    $filterstr = "SELECT * FROM `lead` " . $where . " and isNewInquiry='0'   order by  leadId desc";
    $countstr = "SELECT count(*) as TotalRow FROM `lead`  " . $where . " and isNewInquiry='0'  ";
