...
if ($_POST['action'] == 'ListUser') {
    ...
    $filterstr = "SELECT * FROM `studentcourse`  " . $where . " and  studentcourse.stud_id=" . $_REQUEST['stud_id'];
    $countstr = "SELECT count(*) as TotalRow FROM studentcourse " . $where . " and studentcourse.stud_id=" . $_REQUEST['stud_id'];
            ...
            <tbody>
                ...
                while ($rowfilter = mysqli_fetch_array($resultfilter)) {
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="form-group form-md-line-input">
                                <a  class="btn blue" href="<?php echo $web_url; ?>admin/StudentFeePayHistory.php?token=<?php echo $rowfilter['stud_id']; ?>&studentcourseId=<?php echo $rowfilter['studentcourseId']; ?>"  title="Fee History"><i class="fa fa-database"></i><strong> Fees History</strong></a>
                                ...
                                <a  class="btn blue" href="<?php echo $web_url; ?>admin/StudentFeesHistory.php?token=<?php echo $rowfilter['stud_id']; ?>&studentcourseId=<?php echo $rowfilter['studentcourseId']; ?>"   title="Emi Chart"><i class="fa fa-history iconshowFirst"></i><strong> Emi Chart </strong></a>
                                <?php if ($rowfilter['istatus'] == 0) { ?>                                
                                    <a  class="btn red" onClick="javascript: return deletedata('Active', '<?php echo $rowfilter['stud_id']; ?>', '<?php echo $rowfilter['studentcourseId']; ?>');"   title="Delete"><i class="fa fa-check"></i> Inactive </a>
                                <?php } else { ?>
                                    <a class="btn green" onClick="javascript: return deletedata('Inactive', '<?php echo $rowfilter['stud_id']; ?>', '<?php echo $rowfilter['studentcourseId']; ?>');"  title="Edit"><i class="fa fa-check iconshowFirst"></i> Active</a> 
                                <?php } ?>
                                <button type="button" class="btn blue" value="<==>"  title="EDIT FEE DISCOUNT" onclick="window.open('<?php echo $web_url; ?>admin/StudentFeeDiscount.php?token=<?php echo $rowfilter['stud_id']; ?>&refToken=<?php echo $rowfilter['studentcourseId']; ?>', 'popUpWindow', 'height=500,width=1250,left=100,top=100,resizable=yes,scrollbars=yes,toolbar=yes,menubar=no,location=no,directories=no,status=yes');"><i class="fa fa-rupee"></i> EDIT FEE DISCOUNT</button>
                            </div>
                        </td>
                    </tr>
                <?php } ?>
...
if ($_REQUEST['action'] == 'Active') {
    ...
    $where = " where stud_id='" . $_REQUEST['ID'] . "' and studentcourseId ='" . $_REQUEST['studentcourseId'] . "'";
...
//
////    $dealer_res1 = mysqli_query($dbconn,"delete from studentemidetail where stud_id='" . $stud_id . "' and studentcourseId='" . $studentcourseId . "'");
...
//    $dealer_res = mysqli_query($dbconn,"update studentcourse SET istatus=0 where stud_id='" . $stud_id . "' and studentcourseId='" . $studentcourseId . "'");
//}
?>
