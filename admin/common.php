...
class connect {
    public $conn;
// public function __construct($dbhost,$dbUser,$dbPass,$dbName,$dbconn)
// 	{
// 		$this->conn = mysqli_connect($dbhost,$dbUser,$dbPass,$dbName) or die("Couldn't connection to $host");
...
//    }
    ...
    function sendmail($detail, $giveorder, $sub = 'Message for PD_Astute') {
        $mail = new PHPMailer();
        try {
// $mail->IsSMTP();
//        $mail->Host = "mail.astutemanagement.co.in";
//            $mail->SMTPAuth = true;
//            $mail->Username = "no-reply@astutemanagement.co.in";
//            $mail->Password = "krishna143";
//            $mail->SMTPSecure = 'tls';
//            $mail->Port = 587;
//            $mail->From = "no-reply@astutemanagement.co.in";
//            $mail->FromName = "CCPA";
//            $mail->AddReplyTo("no-reply@astutemanagement.co.in");
//            $mail->AddAddress($giveorder);
//            $mail->IsHTML(true);
//            $mail->Subject = $sub;
//            $mail->Body = $detail;
...
//$mail->SMTPDebug = 4;  
            $mail->IsSMTP();
            $mail->Host = "mail.getdemo.in";
            $mail->SMTPAuth = true;
            $mail->Username = "info@getdemo.in";
            $mail->Password = "info@123";
            $mail->SMTPSecure = 'tls';                            // Enable TLS encryption, `ssl` also accepted
            $mail->Port = 587;

            $mail->From = "no-replay@getdemo.in";
            $mail->FromName = "PD_Astute";
            $mail->AddReplyTo("no-replay@getdemo.in");
//$mail->AddAddress("shahbhavi64@gmail.com");
            $mail->AddAddress($giveorder);
            $mail->addBCC();
            $mail->IsHTML(true);
            $mail->Subject = $sub;
...
//var_dump($res_ofmail);
    ...
    function sendmailMulti($detail, $giveorder, $sub = 'Message for PD_Astute', $CEEmailList) {
        $mail = new PHPMailer();
        try {
// $mail->IsSMTP();
//        $mail->Host = "mail.astutemanagement.co.in";
...
//$mail->AddAddress("shahbhavi64@gmail.com");
            $mail->AddAddress($giveorder);
            for ($i = 1; $i <= count($CEEmailList); $i++) {

                $mail->addBCC($CEEmailList[$i]);
            }
            $mail->IsHTML(true);
            $mail->Subject = $sub;
            $mail->Body = $detail;
...
//}
    ...
    function sendmultisms($mobileNo, $message, $sendername) {
        //$message = $message . ' www.expertdentist.in';

        $data = "http://59.144.126.28/smsserver/SMS10N.aspx?Userid=kshitij&UserPassword=kshitij@123@&PhoneNumber=" . $mobileNo . "&GSM=" . $sendername . "&Text=" . urlencode($message) . "";
    ...
    function sendsinglesms($mobileNo, $message, $sendername) {
