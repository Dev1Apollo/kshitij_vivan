...
//    $whereEmpId = " and studentadmission.employeeMasterId = '" . $_SESSION['EmployeeId'] . "'";
    ...
    if($_REQUEST['studentStatus'] != NULL && isset($_REQUEST['studentStatus']))
        $where .= " and studentadmission.iStudentStatus in (".implode (',',$_REQUEST['studentStatus']).") and studentadmission.iStudentStatus in (select studentstatus.studstatusid from studentstatus where isDelete=0)";
    ...
    $filterstr = "SELECT studentadmission.*,studentfee.*,studentcourse.* from studentfee, studentadmission,studentcourse where studentadmission.stud_id=studentfee.stud_id and studentcourse.stud_id=studentadmission.stud_id " . $where . " and studentcourse.istatus=1 and studentcourse.courseId in (SELECT courseId from course) GROUP BY studentcourse.stud_id,studentcourse.courseId";
    $countstr = "SELECT count(DISTINCT studentcourse.stud_id) as TotalRow, studentadmission.*,studentfee.*,studentcourse.* from studentfee, studentadmission,studentcourse where studentadmission.stud_id=studentfee.stud_id and studentcourse.stud_id=studentadmission.stud_id " . $where . "  and studentcourse.istatus=1 and studentcourse.courseId in (SELECT courseId from course)";
...
//                                    echo $rowSourse['inquirySourceName'];
                            ...
                            <td>
                                <div class="form-group form-md-line-input "><?php
                                    $filterfee = "select sum(amount) as recievedfee from studentfee where stud_id='" . $rowfilter['stud_id'] . "' and studentcourseId='" . $rowfilter['studentcourseId'] . "' ORDER by `payDate` DESC";
