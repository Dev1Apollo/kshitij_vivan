...
if ($_POST['action'] == 'ListUser') {
    ...
    if ($_REQUEST['Employee'] != NULL && isset($_REQUEST['Employee']))
        $where.=" and employeeMasterId='" . $_POST['Employee'] . "'";
...
//echo $filterstr;exit;
    ...
    $countstr = "SELECT count(*) as Inqcount,employeeMasterId ,(select employeemaster.employeeName from employeemaster where employeemaster.employeeMasterId = lead.employeeMasterId) as EmployeeName ,sum((select count(*) from lead l1 where l1.leadId = lead.leadId and lead.statusId='3' ".$where." )) as bookedInq ,"
            . "sum((select l1.booking_amount from lead l1 where l1.leadId = lead.leadId and l1.statusId=3 ".$where." )) as bookingamount,sum((select count(*) from lead l1 where l1.leadId = lead.leadId and l1.walkin_datetime != '' ".$where." )) as walkininquiry  FROM `lead` where 1=1  " . $where . "   GROUP by employeeMasterId";
