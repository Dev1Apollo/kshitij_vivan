...
//                                                                startDate: date
        ...
        $(document).ready(function () {
            ...
            $('#calendar').zabuto_calendar({
                ...
                data: [
<?php
$get_count = mysqli_query($dbconn,"select * from lead where statusId in ('1','6') and employeeMasterId = '" . $_SESSION['EmployeeId'] . "'");


while ($fetch_count = mysqli_fetch_array($get_count)) {
    $get_count_total = mysqli_fetch_array(mysqli_query($dbconn,"select COUNT(*) as totalcount from lead where statusId in ('1','6') and STR_TO_DATE(nextFollowupDate,'%d-%m-%Y') =  STR_TO_DATE('" . $fetch_count['nextFollowupDate'] . "','%d-%m-%Y') and employeeMasterId = '" . $_SESSION['EmployeeId'] . "'"));
    ?>
                        {
