...
if ($_POST['action'] == 'ListUser') {
    ...
    $filterstr = "SELECT studentfee.*,studentadmission.* FROM studentfee,studentadmission where studentadmission.stud_id = studentfee.stud_id " . $where . " and studentfee.amount!=0 and deposit = 'No' and studentfee.paymentMode in (1,2)  order by STR_TO_DATE(payDate,'%d-%m-%Y') asc";
    $countstr = "SELECT count(*) as TotalRow FROM `studentfee` ,studentadmission  where studentadmission.stud_id = studentfee.stud_id " . $where . "  and studentfee.amount!=0 and deposit = 'No' and studentfee.paymentMode in (1,2) ";
