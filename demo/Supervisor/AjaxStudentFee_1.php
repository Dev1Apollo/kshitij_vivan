...
if ($_POST['action'] == 'ListUser') {
    ...
    if ($_REQUEST['courseName'] != NULL && isset($_REQUEST['courseName']))
        $where .= " and  course.courseId = " . $_REQUEST[courseName];

    $filterstr = "SELECT * FROM `studentfee` join studentcourse on studentcourse.studentcourseId=studentfee.studentcourseId join course on course.courseId=studentcourse.courseId  " . $where . " order by studentfeeid desc";

    $countstr = "SELECT count(*) as TotalRow FROM `studentfee` join studentcourse on studentcourse.studentcourseId=studentfee.studentcourseId join course on course.courseId=studentcourse.courseId " . $where . " order by STR_TO_DATE(studentfee.payDate,'%d-%m-%Y') desc";
            ...
            <table class="table table-bordered table-hover center table-responsive dt-responsive" width="100%" id="tableC">
                <thead class="tbg">
                    <tr>
                        ...
                        <th class="desktop">GST Amount</th>
                        <th class="desktop">Tax Free Amount</th>
...
if ($_REQUEST['action'] == 'Delete') {
    ...
    $filteremidetail = "SELECT * FROM studentemidetail WHERE `studentfeeid`='" . $studentfeeid . "' ORDER BY studemiId DESC";
    ...
    while ($emidata = mysqli_fetch_array($row)) {
        ...
        if ($totalAmt == $actualReceivedAmount) {
            $data = array(
        ...
        if ($totalAmt <= $actualReceivedAmount) {
            $diffAmt = $actualReceivedAmount - $totalAmt;
            $dataEmi = array(
                "actualReceivedAmount" => $diffAmt,
                "isPaid" => '0',
                "emiReceivedDate" => "",
                "comments" => "",
                "studentfeeid" => $studentfeeid
