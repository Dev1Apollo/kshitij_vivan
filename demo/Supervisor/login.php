...
if (isset($_POST['btnsubmit']) && isset($_POST['loginId']) && !empty($_POST['loginId'])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if (validate_password($_REQUEST['password'], $good_hash)) {
            if ($row['iEmployeeType'] == 2) {
                $_SESSION['SuperEmployeeId'] = $row['employeeMasterId'];
                $_SESSION['SuperEmployeeName'] = $row['employeeName'];
                $_SESSION['branchid'] = 0;
                $_SESSION['branchname'] = "";
                $_SESSION['Type'] = "2"; //$_POST['Type'];
                $_SESSION['EmployeeType'] = 'Supervisor';
            } else {
            ...
            echo "<script>window.location.href='" . $web_url . "Supervisor/index.php';</script>";
        } else {
            header('location:' . $web_url . 'Supervisor/login.php?flg=1');
            // $message = "UserName or password does't match.";
        }
    } else {
        header('location:' . $web_url . 'Supervisor/login.php?flg=2');
    }
}
?>
<html lang="en">
...
<head>
    ...
    <link href="<?php echo $web_url; ?>Supervisor/assets/global/plugins/font-awesome/css/font-awesome.min.css" rel="stylesheet" type="text/css" />

    <link href="<?php echo $web_url; ?>Supervisor/assets/global/plugins/bootstrap/css/bootstrap.min.css" rel="stylesheet" type="text/css" />

    <link href="<?php echo $web_url; ?>Supervisor/assets/pages/css/login.css" rel="stylesheet" type="text/css" />
...
<body class=" login">
    ...
    <script src="<?php echo $web_url; ?>Supervisor/assets/global/plugins/jquery.min.js" type="text/javascript"></script>
    <script src="<?php echo $web_url; ?>Supervisor/assets/global/plugins/bootstrap/js/bootstrap.min.js" type="text/javascript"></script>

    <script src="<?php echo $web_url; ?>Supervisor/assets/global/scripts/app.js" type="text/javascript"></script>
    <script src="<?php echo $web_url; ?>Supervisor/assets/pages/scripts/login.js" type="text/javascript"></script>
