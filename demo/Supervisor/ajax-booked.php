...
if ($_POST['action'] == 'ListUser') {
    ...
    $filterstr = "SELECT * FROM `lead`  " . $where . " and isNewInquiry='0' and  statusId in ('3') order by STR_TO_DATE(nextFollowupModifyDate, '%d-%m-%Y') asc";
    $countstr = "SELECT count(*) as TotalRow,avg(DATEDIFF(STR_TO_DATE(nextFollowupModifyDate,'%d-%m-%Y'),STR_TO_DATE(inquiryEnterDate,'%d-%m-%Y'))) AS timediff,sum(booking_amount) as bookedamount FROM `lead`  " . $where . " and  isNewInquiry='0' and statusId in ('3')  ";
...
?>
        <div class="row m-search-box">
            <div class="col-md-12">
                <div class="row">
                    <h4 class="col-md-4"><span class="bold" style="color: #51c6dd">Count:</span>&nbsp;&nbsp;<?php echo $get_count; ?></h4>
                    <h4 class="col-md-4"><span class="bold" style="color: #51c6dd">Average Days:</span>&nbsp;&nbsp;<?php echo round($resrowc['timediff'], 2) . ' Days'; ?></h4>
                    <h4 class="col-md-4"><span class="bold" style="color: #51c6dd">Total Amount:</span>&nbsp;&nbsp;<?php echo $resrowc['bookedamount']; ?></h4>
                </div>
            </div>
        </div>
