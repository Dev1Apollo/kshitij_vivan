...
if ($_POST['action'] == 'ListUser') {
    ...
    // and lead.employeeMasterId='" . $_SESSION['EmployeeId'] . "'
    $filterstr = "SELECT customerentry.*,lead.* FROM `lead`  left join customerentry on lead.customerEntryId = customerentry.customerEntryId  " . $where . " and isNewInquiry='0'  and NOT statusId in ('2','3','4') order by STR_TO_DATE(inquiryEnterDate, '%d-%m-%Y') desc";
    //echo $filterstr;exit;
    $countstr = "SELECT count(*) as TotalRow FROM `lead`  " . $where . " and  isNewInquiry='0' and NOT statusId in ('2','3','4')  ";
