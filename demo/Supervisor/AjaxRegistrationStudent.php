...
if ($_POST['action'] == 'ListUser') {
    ...
    if ($_SESSION['EmployeeType'] == 'Supervisor') {
        if ($_POST['employeeMasterId'] != NULL && isset($_POST['employeeMasterId']))
            $where .= " and lead.employeeMasterId='" . $_POST['employeeMasterId'] . "'";
    } else {
        $where .= " and lead.employeeMasterId='" . $_SESSION['EmployeeId'] . "'";
    }
    $filterstr = "SELECT * FROM `lead` inner join customerentry on lead.customerEntryId=customerentry.customerEntryId " . $where . " and lead.statusId = '3' and lead.isRegister = '0' and customerentry.istatus = '1' and customerentry.isDelete = '0' order by STR_TO_DATE(nextFollowupModifyDate,'%d-%m-%Y') DESC";

    $countstr = "SELECT count(*) as TotalRow FROM `lead` inner join customerentry on lead.customerEntryId=customerentry.customerEntryId " . $where . " and lead.statusId = '3' and customerentry.istatus = '1'  and customerentry.isDelete = '0' and isRegister=0";
        ...
        <table class="table table-bordered table-hover center table-responsive dt-responsive" width="100%" id="tableC">
            ...
            <tbody>
                ...
                while ($rowfilter = mysqli_fetch_array($resultfilter)) {
                    ...
                    $serial++;
                    $employeeMaster = [];
                    // if(isset($rowfilter['employeeMasterId']) && $rowfilter['employeeMasterId'] > 0){
                    //     $employeeMaster = mysqli_fetch_array(mysqli_query($dbconn, "select * from employeemaster where employeeMasterId = '" . $rowfilter['employeeMasterId'] . "'"));
                    // }
                ?>
                    <tr>
                        <td>
                        ...
                        <!--<td>-->
                        <!--    <div class="form-group form-md-line-input "><?php echo !empty($employeeMaster) ? $employeeMaster['employeeName'] : '-'; ?> -->
                        <!--    </div>-->
                        <!--</td>-->
                        <td>
                        ...
                        <td>
                            <div class="form-group form-md-line-input">
                                <a class="btn blue" href="<?php echo $web_url; ?>Supervisor/AddNewRegister.php?token=<?php echo $rowfilter['leadId']; ?>&cid=<?php echo $rowfilter['customerEntryId']; ?>" title="STUDENT REGISTRATION"><i class="fa fa-user"></i></a>
                            </div>
                        </td>
                    </tr>
                <?php } ?>
