...
if ($_POST['action'] == 'ListUser') {
	...
	$filterstr = "SELECT *,(select company.strCompanyName from company where company.id=sj.iCompanyId) as strCompanyName FROM `studentadmission` sa inner join studentjobsubmission sj on sa.stud_id=sj.iStudId inner join jobmaster j on j.iJobId=sj.iJobId inner join company on company.id=sj.iCompanyId " . $where . " and sa.isDelete='0' and  sa.istatus='1' and sa.iJobStatus='0' and sj.iJobStatus=0 order by sa.stud_id desc";
	$countstr = "SELECT count(*) as TotalRow FROM `studentadmission` sa inner join studentjobsubmission sj on sa.stud_id=sj.iStudId  inner join jobmaster j on j.iJobId=sj.iJobId inner join company on company.id=sj.iCompanyId " . $where . " and sa.isDelete='0' and sa.istatus='1' and sa.iJobStatus='0' and sj.iJobStatus=0";
