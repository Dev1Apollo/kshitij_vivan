...
if ($_SESSION['EmployeeType'] == 'Supervisor') {
    if ($_REQUEST['branchid'] != NULL && isset($_REQUEST['branchid']))
        $whereid .= " and studentcourse.branchId='" . $_REQUEST['branchid'] . "'";
} else {
    $whereid .= " and studentcourse.branchId=" . $_SESSION['branchid'] . "'";
}

$filterstr = "SELECT studentcourse.`stud_id`,studentcourse.`studentcourseId`,studentfee.chequeNo,studentfee.paymentMode,studentfee.bankName,studentcourse.courseId,studentfee.depositDate,studentfee.`payDate` , studentfee.`iGstRef` ,studentcourse.branchId,
...
if (mysqli_num_rows($query) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($query)) {
        ...
        $Total[5] = $row['taxFreeAmount'] * 1 + $Total[5] * 1;
        $Total[6] = $CGST * 1 + $Total[6] * 1;
        $Total[7] = $SGST * 1 + $Total[7] * 1;
        $Total[8] = $row['netamount'] * 1 + $Total[8] * 1;
