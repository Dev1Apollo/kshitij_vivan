...
?>
		...
		<link href="<?php echo $web_url; ?>Supervisor/assets/global/plugins/datatables/plugins/bootstrap/datatables.bootstrap.css" rel="stylesheet" type="text/css" />
		...
		<table class="table table-bordered table-hover center table-responsive" width="100%" id="tableC">
			...
			<tbody>
				...
				?>
					<tr>
						<!-- <td>
								<div class="md-checkbox">
									<input type="hidden" name="iJobId[]" id="smsid<?php echo $i; ?>" class="md-check" value="<?php echo $rowfilter['iJobId']; ?> ">
									<input type="checkbox" name="check_list[]" id="check_list<?php echo $i; ?>" class="md-check" value="<?php echo $rowfilter['iJobId']; ?> ">
            ...
            <td>
                <div class="form-group form-md-line-input "><?php echo date('M-Y', strtotime($rowfilter['strEntryDate'])); ?>
						...
						<td>
							<div class="form-group form-md-line-input ">
								<a class="btn blue" href="addJob.php?id=<?= $rowfilter['id']; ?>" title="Add Job"><i class="fa fa-plus iconshowFirst"></i></a>
							</div>
						</td>
					<?php
				}
