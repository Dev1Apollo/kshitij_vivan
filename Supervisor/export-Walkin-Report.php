...
//    $f = fopen('php://memory', 'w');
    ...
    $f = fopen('php://memory', 'w');
    $fields = array('Sr.No','Month-Year','Walk-in Date','Lead Unique ID','Customer Name','Customer Mobile','Source Of Lead','Branch','Inquiry Status','Remarks');
    ...
    while ($row = mysqli_fetch_assoc($query)) {
        ...
        $employeeMaster = [];
        if(isset($row['employeeMasterId']) && $row['employeeMasterId'] > 0){
            $employeeMaster = mysqli_fetch_array(mysqli_query($dbconn, "select * from employeemaster where employeeMasterId = '" . $row['employeeMasterId'] . "'"));
        }
        $employeeName = !empty($employeeMaster) ? $employeeMaster['employeeName'] : '-';
        ...
        $lineData = array($i,$data = date('M-Y', $walkin),$row['walkin_datetime'],$row['leaduniqueid'], $rowCustomer['title'] . ' ' . $rowCustomer['firstName'] . ' ' . $rowCustomer['MiddleName'] . ' ' . $rowCustomer['lastName'],$rowCustomer['mobileNo'],$get_source['inquirySourceName'],$employeeName, $inquiryStatus['statusName'],  trim($get_Remark['comment']));
