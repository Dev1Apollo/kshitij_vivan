...
if ($_POST['action'] == 'ListUser') {
	...
	$countstr = "SELECT count(*) as TotalRow FROM studentjobsubmission sjs left join jobmaster jm on sjs.iJobId=jm.iJobId " . $where . " and sjs.iStatus=1 and sjs.iJobStatus=1";
			...
			<table class="table table-bordered table-hover center  dt-responsive" width="100%" id="tableC">
				<thead class="tbg">
					<tr>
						...
						<th class="desktop">Student Contact</th>
						<th class="desktop">Student Email</th>
				...
				<tbody>
					...
					while ($rowfilter = mysqli_fetch_array($resultfilter)) {
						$i++;
						$serial++;
						$Student = mysqli_fetch_array(mysqli_query($dbconn, "SELECT *,(select branchmaster.branchname from branchmaster where branchmaster.branchid=studentadmission.branchId) as 'branchname' FROM `studentadmission` where stud_id=" . $rowfilter['iStudId']));
					?>
						<tr>
							...
							<td>
								<div class="form-group form-md-line-input "><?php echo date('M-Y', strtotime($rowfilter['strPlacementDate'])); ?>
								</div>
							</td>
							<td>
								<div class="form-group form-md-line-input "><?php echo $Student['branchname']; ?>
								</div>
							</td>
							<td>
								<div class="form-group form-md-line-input "><?php echo $Student['title'] . ' ' . $Student['firstName'] . ' ' . $Student['middleName'] . ' ' . $Student['surName']; ?>
								</div>
							</td>
							<td>
								<div class="form-group form-md-line-input "><?php echo $Student['mobileOne']; ?>
								</div>
							</td>
							<td>
								<div class="form-group form-md-line-input "><?php echo $Student['email']; ?>
								</div>
							</td>
