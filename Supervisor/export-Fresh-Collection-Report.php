...
if ($_SESSION['EmployeeType'] == 'Supervisor') {
    if (isset($_REQUEST['branchid']) && $_REQUEST['branchid'] != ""){
        $where .= " and studentadmission.branchId='" . $_REQUEST['branchid'] . "'";
    }
} 
// else {
//     $whereEmpId .= " and studentadmission.branchId=" . $_SESSION['branchid'] . "'";
...
if (mysqli_num_rows($query) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($query)) {
        ...
        $Total[8] = $row['TextFree'] * 1 + $Total[8] * 1;
        $Total[9] = $cgst * 1 + $Total[9] * 1;
        $Total[10] = $sgst * 1 + $Total[10] * 1;
        $Total[11] = $row['Amount'] * 1 + $Total[11] * 1;
