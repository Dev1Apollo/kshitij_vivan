...
switch ($action) {
    ...
    case "AddCreateInquiry":
        if ($_POST['InquiryStatus'] == '6' || $_POST['InquiryStatus'] == '7') {
        ...
        $data_uniqueid = array(
            'leaduniqueid' => 'KV/' . $branchAbbName['AbbreviationName'] . '/' . $dealer_res . '/' . date('m') . '/' . date('Y')
        );
    ...
    case "AddInquiryFollowUp":
        ...
        $statusCount = $rowfilterLF['count'];
        $rowfilterinquirystatus = mysqli_fetch_array(mysqli_query($dbconn, "SELECT * FROM `lead`  where leadId='" . $_POST['leadId'] . "'"));
        if (($_POST['InquiryStatus'] == '6' || $_POST['InquiryStatus'] == '7') && $rowfilterinquirystatus['walkin_datetime'] == '') {
            $walkindate = date('d-m-Y H:i:s');
        } else {
            $walkindate = $rowfilterinquirystatus['walkin_datetime'];
        }
    ...
    case "AddStudentCourse":
        ...
        $findMaxstudId = mysqli_fetch_array(mysqli_query($dbconn, "SELECT MAX(EnrollmentId) as EnrollmentId FROM studentadmission where branchId = " . $_SESSION['branchid'] . " and studentPortal_Id=" . $_POST['iEnroll'] . " "));
...
//        break;
    ...
    case "DepositDetail":
        ...
        foreach ($CheckList as $key => $value) {
            ...
            if ($filetrBank['isGst'] == 'YES') {
                $filterStudentFee = mysqli_fetch_array(mysqli_query($dbconn, "select max(iGstCount) as GstCount from studentfee,studentcourse where studentcourse.studentcourseId=studentfee.studentcourseId and studentcourse.branchId='" . $_SESSION['branchid'] . "'"));
    ...
    case "AddTarget":
        ...
        $filterTarget = mysqli_query($dbconn, "select * from target where month = " . $month . " and year = " . $year . " and iBranchId = " . $_SESSION['branchid'] . " and isDelete = 0 and iStatus=1");
