...
<body class="page-container-bg-solid page-boxed">
	...
	<script type="text/javascript">
		...
		function PageLoadData(Page) {
			...
			$.ajax({
				type: "POST",
				url: "<?php echo $web_url; ?>Employee/AjaxNotRequiredStudentList.php",
				data: {
					action: 'ListUser',
					Page: Page,
		...
		function SetStudentData(StudId) {
			$("#stud_id").val(StudId);
			var errMsg = 'Are you sure to sent into required list?';
			if (confirm(errMsg)) {
				$('#loading').css("display", "block");
				$.ajax({
					type: 'POST',
					url: '<?php echo $web_url; ?>Employee/AjaxNotRequiredStudentList.php',
					data: {action: 'sendtorequiredlist', stud_id: StudId},
