...
//        $filterEmi = mysqli_query($dbconn, "select * from studentemidetail where studentcourseId='" . $dataresult['studentcourseId'] . "' and  stud_id='" . $dataresult['stud_id'] . "' and isDelete=0 ORDER by STR_TO_DATE(emiDate,'%d-%m-%Y') ASC limit 1");
//
//        $i = 0;
...
//if (mysqli_num_rows($result1) > 0) {
    $filename = 'Projction-Report_' . date('d-m-Y_H:s:i') . '.xls';
    ...
    while ($iCounter < sizeof($array)) {
        $ValueField = "";
        if (isset($array[$kCounter][0]) && $array[$kCounter][0] != "") {
            if ($iCounter == 0) {
                $ValueField .= (isset($array[$iCounter][$jCounter]) ? $array[$iCounter][$jCounter] : 0);
            } else {
                $ValueField .= (isset($array[$iCounter][$jCounter]) ? $array[$iCounter][$jCounter] : 0);
            }
            for ($jCounter = 0; $jCounter < sizeof($array[$kCounter]); $jCounter++) {
                ...
                if ($jCounter > 1) {
                    if(isset($TotalArray[$jCounter]))
                    if ($TotalArray[$jCounter - 3] == "" || $TotalArray[$jCounter - 3] == null || $TotalArray[$jCounter - 3] == 'null') {
                        $TotalArray[$jCounter - 3] = 0;
                    }
                    $TotalArray[$jCounter - 3] = (isset($TotalArray[$jCounter - 3]) ? $TotalArray[$jCounter - 3] : 0) + (is_numeric($array[$kCounter][$jCounter]) ? $array[$kCounter][$jCounter] : 0);
                }
            }
            if ($iCounter == 0) {
                $ValueField .= "";
    ...
    for ($jCounter = 0; $jCounter < sizeof($TotalArray) - 3; $jCounter++) {
        if ($jCounter == 0) {
            echo "Total";
        } else {
            echo "\t" .(isset($TotalArray[$jCounter - 3]) ? $TotalArray[$jCounter - 3] : 0);
        }
    }
