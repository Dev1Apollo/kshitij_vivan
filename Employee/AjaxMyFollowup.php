...
if ($_POST['action'] == 'ListUser') {
    ...
    if ($_REQUEST['FormDate'] != NULL && isset($_REQUEST['FormDate']))
        $where.=" and STR_TO_DATE(nextFollowupDate,'%d-%m-%Y')>= STR_TO_DATE('" . $_REQUEST['FormDate'] . "','%d-%m-%Y')";
    else
        $where.=" and DATE_FORMAT(STR_TO_DATE(nextFollowupDate, '%d-%m-%Y'), '%Y-%m-%d')=STR_TO_DATE('" . date('d-m-Y') . "','%d-%m-%Y')";
    if ($_REQUEST['ToDate'] != NULL && isset($_REQUEST['ToDate']))
        $where.=" and DATE_FORMAT(STR_TO_DATE(nextFollowupDate, '%d-%m-%Y'), '%Y-%m-%d')<=STR_TO_DATE('" . $_REQUEST['ToDate'] . "','%d-%m-%Y')";
    if ($_REQUEST['InquiryStatus'] != NULL && isset($_REQUEST['InquiryStatus']))
        $where.=" and statusId='" . $_REQUEST['InquiryStatus'] . "'";

//    if ($_REQUEST['CategoryOfInquiry'] != NULL && isset($_REQUEST['CategoryOfInquiry']))
//        $where.=" and categoryOfInquiry='" . $_REQUEST['CategoryOfInquiry'] . "'";
    if ($_REQUEST['inquirySourceName'] != NULL && isset($_REQUEST['inquirySourceName']))
        $where.=" and customerentry.inquirySourceId='" . $_REQUEST['inquirySourceName'] . "'";
    ...
    $filterstr = "SELECT customerentry.*,lead.* FROM `lead`  left join customerentry on lead.customerEntryId = customerentry.customerEntryId  " . $where . " and isNewInquiry='0' and lead.employeeMasterId='" . $_SESSION['EmployeeId'] . "' and NOT statusId in ('2','3','4') order by STR_TO_DATE(inquiryEnterDate, '%d-%m-%Y') desc";
    //echo $filterstr;exit;
    $countstr = "SELECT count(*) as TotalRow FROM `lead`  " . $where . " and  isNewInquiry='0' and employeeMasterId='" . $_SESSION['EmployeeId'] . "' and NOT statusId in ('2','3','4')  ";
...
//                                echo $rowfilterLF['count'];
            ...
            <!--                        <td>
                            <div class="form-group form-md-line-input "><?php
                        $inquiryCategory = mysqli_fetch_array(mysqli_query($dbconn,"SELECT * FROM `categoryofinquiry`  where isDelete='0'  and  istatus='1' and id='" . $rowfilter['categoryOfInquiry'] . "'"));
                        echo $inquiryCategory['COIname'];
                        ?> 
                            </div>
                        </td>-->
                        <td>
                        ...
                        <td style="">
                            <div class="form-group form-md-line-input">
                                <?php if ($inquiryStatus['statusName'] == 'BOOKED' || $inquiryStatus['statusName'] == 'LOST') { ?>
                                <?php } else { ?>
