...
if ($_POST['action'] == 'ListUser') {
    ...
    $filterstr = "SELECT * FROM `lead`  " . $where . " and isNewInquiry='0' and employeeMasterId='" . $_SESSION['EmployeeId'] . "' and  statusId in ('3') order by STR_TO_DATE(nextFollowupModifyDate, '%d-%m-%Y') asc";
    $countstr = "SELECT count(*) as TotalRow,avg(DATEDIFF(STR_TO_DATE(nextFollowupModifyDate,'%d-%m-%Y'),STR_TO_DATE(inquiryEnterDate,'%d-%m-%Y'))) AS timediff,sum(booking_amount) as bookedamount FROM `lead`  " . $where . " and  isNewInquiry='0' and employeeMasterId='" . $_SESSION['EmployeeId'] . "' and statusId in ('3')  ";
